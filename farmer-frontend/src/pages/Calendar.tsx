...
export default function Calendar() {
  ...
  // Load minimal data for event computation (farmer-only calendar)
  useEffect(() => {
    let cancelled = false;
    async function load() {
      ...
      setPlants(pl);
      setHarvests(hv);
      // Farmer: start with empty custom events; user can add their own
      setCustomEvents([]);
      // Use shared mock operators list for consistency (locations/facilities)
  ...
  // Combine system-computed cultivation events with user scheduled items
  const events = useMemo(() => {
    // Farmer: include all computed cultivation events
    const computed = computeEventsForCannabis(plants, harvests);
    return [
      ...computed,
      ...customEvents,
    ] as any[];
      ...
      className="inline-flex items-center gap-1 px-2 py-1 border rounded-md text-sm text-gray-800 hover:bg-gray-50"
            ...
            {addOpen && (
              <div className="absolute right-0 mt-2 w-64 rounded-lg border border-gray-200 bg-white shadow-lg py-1 z-50">
                {[
                  { label: 'Planting', hint: 'Germination or direct seed' },
                  { label: 'Transplant', hint: 'Move to bed/room/greenhouse' },
                  { label: 'Flip to flower', hint: 'Change light schedule' },
                  { label: 'Harvest', hint: 'Planned harvest date' },
                  { label: 'Lab submission', hint: 'Send samples to lab' },
                  { label: 'COA due', hint: 'Certificate of analysis deadline' },
                  { label: 'Nutrient/IPM', hint: 'Nutrient or IPM application' },
                  { label: 'Packaging run', hint: 'Prepare SKUs for sale' },
                  { label: 'Transfer/Shipment', hint: 'Outbound shipment prep' },
                ].map((opt) => (
                  <button
                    key={opt.label}
                    type="button"
