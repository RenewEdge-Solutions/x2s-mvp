...
import { Leaf, Wand2, User as UserIcon, ChevronDown, LogOut, Calendar as CalendarIcon, Bell, FileText, Search, Plus, ShieldCheck, Workflow, Scale, Building2, Package } from 'lucide-react';
        ...
        <div className="ml-auto flex items-center gap-4">
          ...
          <nav className="hidden md:flex items-center gap-1 p-0.5 rounded-lg bg-white/40">
            ...
            <Link
              ...
              to="/production"
            >
              <Workflow className="h-4 w-4" aria-hidden /> Production
            </Link>
            {/* Inventory (next to Production) */}
            <Link
              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-sm transition-colors ${
                isActive('/inventory')
                  ? 'text-primary bg-primary/10 ring-1 ring-primary/20'
          ...
          <div className="flex items-center gap-3">
            <NotificationsMenu />
            <UserMenu
              name={user?.firstName || user?.lastName ? `${user?.firstName} ${user?.lastName}`.trim() : user?.username || 'Farmer'}
...
type Notification = { 
  ...
  category: 'production' | 'compliance' | 'inspection' | 'reports' | 'capacity';
  operator?: string;
...
function NotificationsMenu() {
    ...
    const mock: Notification[] = [
      {
        id: 'prod-001',
        title: 'COA available',
        message: 'Batch B-23-091 received COA: PASS at St. Lucia State Lab.',
        severity: 'info',
        category: 'production',
        operator: 'Farm HQ',
        createdAt: isoAgo(3),
      ...
      {
        id: 'prod-002',
        title: 'COA failed – retest needed',
        message: 'Batch B-23-108 failed microbial limits. Retest recommended.',
        severity: 'alert',
        category: 'production',
        operator: 'Greenhouse 3',
        createdAt: isoAgo(5),
        href: '/lifecycle',
        read: false,
      },
      {
        ...
        message: 'Routine inspection tomorrow at Farm HQ (09:00).',
        severity: 'info',
        category: 'inspection',
        operator: 'Farm HQ',
        createdAt: isoAgo(8),
      ...
      },
      {
        id: 'task-201',
        title: 'Harvest due',
        message: 'Greenhouse 2 - Bed C projected harvest in 2 days (Batch B-23-104).',
        severity: 'warning',
        category: 'production',
        operator: 'Greenhouse 2',
        createdAt: isoAgo(10),
        href: '/calendar',
        read: false,
      },
      {
        id: 'rpt-901',
        title: 'Report ready',
        message: 'Weekly plant inventory has been generated (CSV).',
        severity: 'info',
        category: 'reports',
        createdAt: isoAgo(12),
        href: '/reports',
  ...
  const groupedNotifications = useMemo(() => {
    const groups: NotificationGroup[] = [
      { category: 'production', title: 'Production', items: items.filter(i => i.category === 'production'), priority: 1 },
      { category: 'inspection', title: 'Inspections', items: items.filter(i => i.category === 'inspection'), priority: 2 },
      { category: 'compliance', title: 'Compliance', items: items.filter(i => i.category === 'compliance'), priority: 3 },
      { category: 'reports', title: 'Reports', items: items.filter(i => i.category === 'reports'), priority: 4 },
      { category: 'capacity', title: 'Capacity', items: items.filter(i => i.category === 'capacity'), priority: 5 },
    ];
                      ...
                      <li key={n.id} className="py-2 px-2 hover:bg-gray-50">
                        <div className="flex items-start gap-3">
                          <div className={`mt-0.5 h-6 w-6 rounded-full flex items-center justify-center text-white text-[11px] ${
                            ...
                            {n.category === 'inspection' ? '🗓' : n.category === 'reports' ? '⬇' : n.category === 'capacity' ? '🏗' : n.category === 'compliance' ? '✔' : '🌿'}
                          </div>
...
function SearchBox() {
  ...
  { label: 'Production', path: '/production', keywords: 'plants batches actions packaging transfer' },
  { label: 'Lifecycle', path: '/lifecycle', keywords: 'trace seed-to-sale stages batches' },
  { label: 'Calendar', path: '/calendar', keywords: 'schedule planting harvest lab' },
  { label: 'Reports', path: '/reports', keywords: 'csv pdf inventory yields coa transfers' },
  { label: 'License', path: '/license', keywords: 'licence licensing permit authority regulator' },
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-52 rounded-lg border border-gray-200 bg-white shadow-lg py-1 z-50">
          <button type="button" onClick={() => { navigate('/calendar'); setOpen(false); }} className="w-full text-left px-3 py-2 text-sm text-gray-800 hover:bg-gray-50 inline-flex items-center gap-2">
            <CalendarIcon className="h-4 w-4 text-gray-500" aria-hidden /> Add calendar item
          </button>
          <button type="button" onClick={() => { navigate('/production'); setOpen(false); }} className="w-full text-left px-3 py-2 text-sm text-gray-800 hover:bg-gray-50 inline-flex items-center gap-2">
            <Wand2 className="h-4 w-4 text-gray-500" aria-hidden /> Production action
          </button>
          <button type="button" onClick={() => { navigate('/reports'); setOpen(false); }} className="w-full text-left px-3 py-2 text-sm text-gray-800 hover:bg-gray-50 inline-flex items-center gap-2">
            <FileText className="h-4 w-4 text-gray-500" aria-hidden /> Download report
          </button>
        </div>
      )}
    </div>
