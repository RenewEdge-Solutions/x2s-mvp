...
import { Leaf, User as UserIcon, ChevronDown, LogOut, Calendar as CalendarIcon, Bell, FileText, Search, Plus, ShieldCheck, Workflow, Building2, Boxes } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <NotificationsMenu />
            <UserMenu
              name={user?.firstName || user?.lastName ? `${user?.firstName} ${user?.lastName}`.trim() : user?.username || 'Auditor'}
    ...
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="hidden sm:flex flex-col leading-tight">
          <span className="text-sm font-medium">{name || 'Auditor'}</span>
          <span className="text-[11px] text-gray-500">{role || 'Auditor'}</span>
        </span>
          ...
          <div className="px-3 pb-2">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="text-sm font-medium text-gray-900 truncate">{name || 'Auditor'}</div>
                <div className="text-xs text-gray-500 truncate inline-flex items-center gap-1">
                  <ShieldCheck className="h-3.5 w-3.5 text-emerald-600" aria-hidden />
                  {role || 'Auditor'} • {org || 'Authority'}
                </div>
              </div>
            </div>
          </div>
...
function NotificationsMenu() {
  ...
  const refresh = async () => {
    ...
    const mock: Notification[] = [
      ...
      },
      {
        id: 'coa-771',
        title: 'COA failure',
        message: 'Batch B-24-771 failed residual solvent limits (Acetone).',
        severity: 'alert',
        category: 'compliance',
        operator: 'Sunrise Processing',
        createdAt: isoAgo(3),
      ...
      },
      {
        ...
        message: 'After-hours transfers between Retail and Lab detected (2 events).',
        severity: 'warning',
        category: 'integrity',
        createdAt: isoAgo(6),
        href: '/integrity',
      ...
      {
        id: 'lab-552',
        title: 'Lab SLA breach',
        message: 'Medicanna Labs exceeded TAT SLA on 3 submissions (median 7.2 days).',
        severity: 'warning',
        category: 'compliance',
      ...
      },
      {
        ...
        message: 'On-chain totals differ from system ledger for a recent batch (delta 1.2%).',
        severity: 'alert',
        category: 'integrity',
        createdAt: isoAgo(10),
        href: '/integrity',
  ...
  const groupedNotifications = useMemo(() => {
    const groups: NotificationGroup[] = [
      ...
      { category: 'integrity', title: 'Integrity', items: items.filter(i => i.category === 'integrity'), priority: 3 },
                      ...
                      <li key={n.id} className="py-2 px-2 hover:bg-gray-50">
                        <div className="flex items-start gap-3">
                          <div className={`mt-0.5 h-6 w-6 rounded-full flex items-center justify-center text-white text-[11px] ${
                            ...
                            {n.category === 'integrity' ? '⛓' : n.category === 'inspection' ? '🗓' : n.category === 'reports' ? '⬇' : n.category === 'capacity' ? '🏗' : '✔'}
                          </div>
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-52 rounded-lg border border-gray-200 bg-white shadow-lg py-1 z-50">
          ...
          {/* Licensing quick action removed for auditor */}
        </div>
      )}
    </div>
  );
