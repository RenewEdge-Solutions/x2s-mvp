...
async function handleMock(path: string, init?: RequestInit): Promise<Response | null> {
  ...
  if (path === '/auth/login' && method === 'POST') return jsonResponse({ token: 'demo', user: { role: 'laboratory' } });
  if (path === '/auth/verify-2fa' && method === 'POST') return jsonResponse({ ok: true });
  if (path === '/coas' && method === 'GET') return jsonResponse([{ id: 'coa-1', batch: 'B-1001', thc: 18.2, cbd: 0.2 }]);
  return null;
}
