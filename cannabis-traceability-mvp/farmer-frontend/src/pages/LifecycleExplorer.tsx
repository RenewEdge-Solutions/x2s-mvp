...
export default function LifecycleExplorer() {
  ...
  // Farmer: no licensing view; category fixed to operator only
  const { activeModule } = useModule();
  useEffect(() => {
    if (activeModule === 'cannabis') {
      api.getLifecycle().then((ops) => {
        const onlyOps = (ops || []).filter((e: any) => getCategory(e) === 'operator');
        onlyOps.sort((a: any, b: any) => (getTimeKey(b) || 0) - (getTimeKey(a) || 0));
        setEvents(onlyOps);
      });
    } else {
      setEvents([]);
  ...
  // Farmer: only operator events
  const cat = getCategory(e);
  const matchesCat = cat === 'operator';
  return matchesType && matchesBatch && matchesResp && matchesOrigin && matchesCat;
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 inline-flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-emerald-600" aria-hidden /> Lifecycle</h1>
        </div>
      </div>
      <Card>
  ...
  {/* Filters moved to header row */}
      ...
      <tr className="bg-white border-b border-gray-100 text-xs">
              <th className="py-1 pr-4">
                <span className="inline-flex items-center rounded-md bg-emerald-50 ring-1 ring-emerald-200 px-2 py-0.5 text-xs text-emerald-700">Operator</span>
              </th>
              <th className="py-1 pr-4">
...
function renderCategoryPill(e: any) {
  const cat = getCategory(e);
  const classes = cat === 'licensing'
    ? 'text-indigo-700 bg-indigo-50 ring-indigo-200'
    : 'text-emerald-700 bg-emerald-50 ring-emerald-200';
  const Icon = cat === 'licensing' ? FileText : Building2;
  const label = cat === 'licensing' ? 'Licensing' : 'Operator';
  return (
