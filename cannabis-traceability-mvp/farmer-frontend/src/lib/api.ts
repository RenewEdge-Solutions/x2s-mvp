...
let mockPlants: Plant[] = [
  { id: 'p-2001', strain: 'Sour Diesel', location: 'Bed A', stage: 'Vegetative', createdAt: new Date().toISOString() },
];
...
async function handleMock(path: string, init?: RequestInit): Promise<Response | null> {
  ...
  if (path === '/auth/login' && method === 'POST') return jsonResponse({ token: 'demo', user: { role: 'farmer' } });
  if (path === '/auth/verify-2fa' && method === 'POST') return jsonResponse({ ok: true });
  if (path === '/plants' && method === 'GET') return jsonResponse(mockPlants);
  if (path === '/plants' && method === 'POST') {
    const b = await readJson(init);
    const p: Plant = { id: `p-${Date.now()}`, strain: b?.strain ?? 'Unknown', location: b?.location ?? 'Bed A', stage: 'Vegetative', createdAt: new Date().toISOString() };
    mockPlants = [p, ...mockPlants];
