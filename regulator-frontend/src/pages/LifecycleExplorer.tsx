...
export default function LifecycleExplorer() {
  ...
  const [categoryFilter, setCategoryFilter] = useState<'all' | 'licensing' | 'operator'>('all');
  const { activeModule } = useModule();
  useEffect(() => {
    if (activeModule === 'cannabis') {
      Promise.all([api.getLifecycle(), api.getLicensingEvents?.() || Promise.resolve([])])
        .then(([ops, lic]) => {
          const combined = [...(ops || []), ...(lic || [])];
          // Sort by event time desc when available
  ...
  {/* Filters moved to header row */}
      ...
      <tr className="bg-white border-b border-gray-100 text-xs">
              <th className="py-1 pr-4">
                <select value={categoryFilter} onChange={e=>setCategoryFilter(e.target.value as any)} className="w-full rounded-md border border-gray-300 px-2 py-1">
